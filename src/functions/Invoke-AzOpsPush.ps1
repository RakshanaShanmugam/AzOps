function Invoke-AzOpsPush {
    ...
    process {
        ...
        $deleteSet = @()
        $deleteSetInclusionList = @()
        $autoGeneratedPathFileName = Get-PSFConfigValue -FullName 'AzOps.Core.AutoGeneratedTemplateFolderPath'
        ...
        if($DeleteSetContents -and $deleteSet) {
            ...
            foreach ($item in $deleteSet) {
                ...
                $inclusionFlag = $false
                foreach($managementGroupName in $allManagementGroups){
                    $path = Split-Path -Path $item
                    $managementGroupName = $managementGroupName.ToLower()
                    if($autoGeneratedPathFileName -ne "."){
                        $path = $path.Replace($autoGeneratedPathFileName,"")
                    }
                    $path = (Split-Path -Path $Path -Leaf).ToLower()
                    
                    if($path -like "$managementGroupName*"){
                        Write-PSFMessage -Level Warning -String 'InvokeAzOpsPush.ManagementGroups.NotSupported' -StringValues $item
                        $inclusionFlag = $true
                        break
                    }
                }
                if(-not $inclusionFlag){
                        $deleteSetInclusionList += $item
                }
            }
            
            $DeleteSetContents = $DeleteSetContents -join "" -split "-- " | Where-Object {$_}
            foreach ($item in $deleteSetInclusionList) {
                Write-PSFMessage @common -String 'Invoke-AzOpsPush.Change.Delete.File' -StringValues $item
        ...
        $deletionList = foreach ($deletion in $deleteSetInclusionList | Where-Object { $_ -match ((Get-Item $StatePath).Name) }) {
